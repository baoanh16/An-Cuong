...
var AjaxCart = {
    ...
    addproducttocart_catalog: function (button) {
        ...
        if (this.effecttocart == true && this.topcartselector) {
            var img = $(button).parent().siblings('.figure-image').find('img') || $('.swiper-product-top .swiper-slide-active img');
    ...
    success_desktop: function (response) {
        let noty = new Noty({
            ...
            layout: 'topRight',
            timeout: 3000,
            text: response.message + ': ' + '<strong>' + $(response.updateflyoutcartsectionhtml).find('.last-added').text() + '</strong>'
        }).show();
        if (response.updatetopcartsectionhtml) {
            $(AjaxCart.topcartselector).html(response.updatetopcartsectionhtml);
        }
